...
namespace MissionPlanner
{
    public class CurrentState : ICloneable, IDisposable
    {
        ...
        public float ch3percent11
        {
            get
            {
                return _ch3percent/100.0f;                
            }
        }
        ...
        //public List<(DateTime time,string message)> messagwran { get; set; } = new List<(DateTime, string)>();
         public Dictionary<int, string> messagerror = new Dictionary<int, string>();
        /// <summary>
        ...
        public void updateErrordata(int key,string str)
        {
            if (messagerror.ContainsKey(key))
            {
                messagerror[key] = str;
            }
            else
            {
                messagerror.Add(key, str);
            }
        }
                ...
                switch (mavLinkMessage.msgid)
                {
...

                    {
                        ...
                        if (fence.breach_status != (byte) MAVLink.FENCE_BREACH.NONE)
                        {
                            ...
                            updateErrordata(1,"Fence Breach");
                        }
                    }
...

                    {
                        ...
                        if (ekfvelv >= 1)
                        {
                            ...
                            updateErrordata(2, Strings.velocity_variance);
                        }

                        if (ekfcompv >= 1)
                        {
                            messageHigh = Strings.ERROR + " " + Strings.compass_variance;
                            updateErrordata(3, Strings.compass_variance);
                        }

                        if (ekfposhor >= 1)
                        {
                            messageHigh = Strings.ERROR + " " + Strings.pos_horiz_variance;
                            updateErrordata(4, Strings.pos_horiz_variance);
                            }

                        if (ekfposvert >= 1)
                        {
                            messageHigh = Strings.ERROR + " " + Strings.pos_vert_variance;
                            updateErrordata(5,Strings.pos_vert_variance);
                        }

                        if (ekfteralt >= 1)
                        {
                            messageHigh = Strings.ERROR + " " + Strings.terrain_alt_variance;
                            updateErrordata(6, Strings.terrain_alt_variance);
                        }
...

                    {
                                 ...
                                 sensors_present.LASER_POSITION)
                        {
                            messageHigh = Strings.BadLiDARHealth;
                                updateErrordata(14, messageHigh);
                            }
                                 ...
                                 sensors_present.optical_flow)
                        {
                            messageHigh = Strings.BadOptFlowHealth;
                                updateErrordata(15, messageHigh);
                            }

...
namespace MissionPlanner.Controls
{
    public class HUD : GLControl
    {
        ...
        private readonly SolidBrush _BlackBrush = new SolidBrush(Color.Red);

        private static readonly SolidBrush SolidBrush = new SolidBrush(Color.FromArgb(0x55, 0xff, 0xff, 0xff));
        private static readonly SolidBrush SolidBrushBack = new SolidBrush(Color.FromArgb(255,79, 86,58));
        ...
        void doPaint()
        {
            ...
            try
            {
                ...
                if (displayrollpitch)
                {
                    ...
                    drawstring(_pitch.ToString("F1"), font, fontsize + 2, _whiteBrush,
                                   centercircle.Left + halfwidth / 9,
                                   0 - 2*font.Height);

                    drawstring(_roll.ToString("F1"), font, fontsize + 2, _BlackBrush,
                                   centercircle.Left + halfwidth + halfwidth / 20,
                                   0  - 2 * font.Height);
                    drawstring(_roll.ToString("F1"), font, fontsize + 2, _BlackBrush,
                                   centercircle.Left -  halfwidth *3/ 20,
                                   0 - 2 * font.Height);
                    
                    int fontsize1 = 12;
                    Rectangle backrect = new Rectangle(-this.Width / 2 + 5, halfheight - 3 * fontsize1 - 10, 230, 3 * fontsize1);
                    Rectangle backrect1 = new Rectangle(this.Width / 2 - 135, halfheight - 3 * fontsize1 - 10, 130, 3 * fontsize1);
                    
                    //path1 = ;
                   graphicsObject.FillPath(SolidBrushBack, GraphicsPathDrawGraphicsPathCradius(backrect, 8));
                  //   graphicsObject.FillRectangle(SolidBrushBack, backrect);   
                    
                    //path2 = ;
                   graphicsObject.FillPath(SolidBrushBack, GraphicsPathDrawGraphicsPathCradius(backrect1,8));
                  //      graphicsObject.FillRectangle(SolidBrushBack, backrect1);

                    string asgs = string.Format("AS: {0:F1}{1}  GS: {2:F1}{3}",
                        _airspeed, speedunit, _groundspeed, speedunit);
                    drawstring(asgs, font, fontsize1, _whiteBrush,
                        -this.Width / 2 + 10,
                        halfheight  - 3 * 12);

                    // mode and wp dist and wp
                    drawstring(_mode, font, fontsize1, _whiteBrush, this.Width / 2 - 130,
                        halfheight - 3 * 12);
                }
        ...
        /// <param name="cradius"> 半径</param>
        /// <returns></returns>
        private GraphicsPath GraphicsPathDrawGraphicsPathCradius(Rectangle rect, int cradius)
        {
            GraphicsPath mypath = new GraphicsPath();

            //var circleRect = new RectangleF(rect.Right, rect.Top, cradius, cradius);
            //mypath.AddArc(circleRect, 0, -90);
            //circleRect.Offset(-rect.Width, 0);
            //mypath.AddArc(circleRect, -90, -90);
            //circleRect.Offset(0, rect.Height);
            //mypath.AddArc(circleRect, -180, -90);
            //circleRect.Offset(rect.Width, 0);
            //mypath.AddArc(circleRect, -270, -90);
            //mypath.CloseFigure();
            //return mypath;

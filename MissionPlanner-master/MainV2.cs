...
namespace MissionPlanner
{
    public partial class MainV2 : Form
    {
        ...
        public MainV2()
        {
            ...
            if (Settings.Instance["YangDa_connetUAVCheck"] != null)
            {
                if ("0" == Settings.Instance["YangDa_connetUAVCheck"].ToString())
                {
                    IsDebugFlag = true;
                }
                else
        ...
        private void RegisterUser()
        {
            ...
            RegistryKey SOFT = hkml.OpenSubKey(@"SOFTWARE", true);
            Object obj = SOFT.GetValue("YangDaUserTime");
            if (null != obj)
            {
                string tmstr = obj.ToString();
                DateTime tm = DateTime.Now;
                DateTime tmold = Convert.ToDateTime(tmstr);

                TimeSpan tmpan = tm - tmold;

                if ((tmpan.Days > 30) || (tmpan.Days < 0))
                {
                    MessboxShowInfo err = new MessboxShowInfo("error", "No authorized version, please contact your service provider!!!", 3);
                    err.ShowDialog();
                    this.Close();
                }
            }
            ...
            {
                SOFT.SetValue("YangDaUserTime", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            }
        }
        /// <summary>
        /// 初始化界面
        ...
        private void IniUI()
        {
            image_flying.Text = "Flying";
            image_plan.Text = "Planning";
            image_setup.Text = "Setup";
            image_help.Text = "Support";
            piclmg_connet.Text = "Connect";

            label_Warning.Text = "";
            ARME_text.Text = "";
            ARME_text.Text = "Waiting";
            ARME_text.ForeColor = Color.Red;

            imageLabel_xinhao.Text = "0";

            addAnimation();
            addPosition();
            //image_flying.BackColor = select_color;
            //Bitmap bmp = new Bitmap(panel_satellite.Width, panel_satellite.Height);
            //Graphics g = Graphics.FromImage(bmp);
            //RectangleF rectangle = new RectangleF(0, 0, panel_satellite.Width, panel_satellite.Height);
            //Draw(rectangle, g, 10);
            //RectImage = bmp;
            //RectImage.Save("d:\\12345.png");
           // panel_arm.BackgroundImage = RectImage;
          //  panel_satellite.BackgroundImage = RectImage;
        ...
        private void MenuFlightData_Click(object sender, EventArgs e)
        {
            ...
            FlightData.SetDrawPolygonDate(FlightPlanner.AddPointsDraw());
            FlightData.selectShow();
        }

        private void MenuFlightPlanner_Click(object sender, EventArgs e)
        {
            MyView.ShowScreen("FlightPlanner");   
            FlightPlanner.panel10_MouseLeave(null, null);           
        }
    ...
    System.Windows.Forms.DialogResult.OK)
        ...
        private void GetUAVwps()
        {
            try                
            {
                var type = MAVLink.MAV_MISSION_TYPE.MISSION;
                var list = Task.Run(async () => await mav_mission
               .download(MainV2.comPort, MainV2.comPort.MAV.sysid,
                MainV2.comPort.MAV.compid, MAVLink.MAV_MISSION_TYPE.FENCE).ConfigureAwait(false)).Result;
            }
            catch (Exception ex)
            {
        ...
        {
            comPort.giveComport = false;
            _connectionControl.CMB_serialport.SelectedIndex = this.comboBox1.SelectedIndex;
            _connectionControl.CMB_baudrate.SelectedIndex = this.comboBox2.SelectedIndex;
            ...
            if (comPort.BaseStream.IsOpen)
            {
                ...
                {
                    FlightData.modifyandSetAltMaxAndMin(999,100);
                    FlightData.modifyandSetLoiterRadMaxAndMin(500, 90);                   
                }
                else
                {
                    FlightData.modifyandSetAltMaxAndMin(1000, 0);
                    FlightData.modifyandSetLoiterRadMaxAndMin(10000, -10000);
                }
                loadph_serial();
                CheckConnet();
            }
        ...
        private void CheckConnet()
        {
            if (IsDebugFlag)
                return;
            if (MainV2.comPort.MAV.param.ContainsKey("RC16_TRIM") &&
                (float)MainV2.comPort.MAV.param["RC16_TRIM"] != 1550)
            {
                using (MessboxShowInfo box = new MessboxShowInfo("Connet error", "Invalid connection denied", 3))
                {
                    box.ShowDialog();
                    doDisconnect(comPort);
                    return;
            ...
            if (MainV2.comPort.MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)//gudingyi
            {
                if (MainV2.comPort.MAV.param.ContainsKey("TKOFF_ALT") &&
               (float)MainV2.comPort.MAV.param["TKOFF_ALT"] != 102)
                {
                    using (MessboxShowInfo box = new MessboxShowInfo("Connet error", "Invalid connection denied", 3))
                    {
                        box.ShowDialog();
        ...
        private void CMB_serialport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Settings.Instance[_connectionControl.CMB_serialport.Text + "_BAUD"] != null)
                {
                    ...
                    comboBox2.Text = _connectionControl.CMB_baudrate.Text;
                }
            }
            catch
            {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (Program.args.Length > 0)
            {
                ...
                if (cmds.ContainsKey("port") && cmds.ContainsKey("baud"))
                {
                    ...
                    comboBox1.Text = _connectionControl.CMB_serialport.Text;
                    comboBox2.Text = _connectionControl.CMB_baudrate.Text;
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                InitialSetup.instance.LoadConfig();
                return true;
            }
        ...
        {
            imageLabel_xinhao.Left     = this.Width - piclmg_connet.Width - imageLabel_xinhao.Width - 20;
            panel_arm.Left       = imageLabel_xinhao.Left - panel_satellite.Width - 10;
         //   panel_arm.Top = panel1.Top + (panel1.Height - panel_arm.Height) / 2;
            pictureBox2.Left   = panel_arm.Left - pictureBox2.Width - 20;
            // panel_arm.Left = panel_satellite.Left - panel_arm.Width - 10;
        }
        private void addAnimation()
        {
            foreach (Control ctl in this.panel1.Controls)
            {
                if (ctl is ImageLabel)
                {
                    if (-1 != ctl.Name.IndexOf("image_"))
                    {
                        (ctl as ImageLabel).MouseEnter += new System.EventHandler(this.FireSet_MouseEnter);
                        (ctl as ImageLabel).MouseLeave += new System.EventHandler(this.FireSet_MouseLeave);
                        (ctl as ImageLabel).Click += new System.EventHandler(this.FireSet_MouseClick);
                    }
                }
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FireSet_MouseEnter(object sender, EventArgs e)
        {
            (sender as ImageLabel).BackColor = select_color;
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FireSet_MouseLeave(object sender, EventArgs e)
        {
            if ((sender as ImageLabel).Name != Selectstr)
                (sender as ImageLabel).BackColor = Normal_color;
            else
                (sender as ImageLabel).BackColor = select_color;
        }
        private string Selectstr = "image_flying";
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FireSet_MouseClick(object sender, EventArgs e)
        {
            ImageLabel btn = sender as ImageLabel;            
            Selectstr = btn.Name;
            foreach (Control ctl in panel1.Controls)
            {
                if (ctl is ImageLabel)
                {
                    if (-1 != ctl.Name.IndexOf("image_"))
                    {
                        if(ctl.Name != Selectstr)
                            (ctl as ImageLabel).BackColor = Normal_color;
                    }
                }
            }
        ...
        private void pic_connet_Click(object sender, EventArgs e)
        {
            panel_NewConnet.Visible = true;
        }
        ...
        private void comboBox1_DropDown(object sender, EventArgs e)
        {
            ...
            {
                _connectionControl.CMB_serialport.Text = oldport;
                this.comboBox1.SelectedIndex = _connectionControl.CMB_serialport.SelectedIndex;
                this.comboBox2.SelectedIndex = _connectionControl.CMB_baudrate.SelectedIndex;
            }
        }
        private void myButton_connet_Click(object sender, EventArgs e)
        {
            this.panel_NewConnet.Visible = false;
            Connect();
          //  timer_warning.Start();
        }
        ...
        private void piclmg_connet_MouseEnter(object sender, EventArgs e)
        {
           // piclmg_connet.BackColor = select_color;
        }

        private void piclmg_connet_MouseLeave(object sender, EventArgs e)
        {
           // piclmg_connet.BackColor = Normal_color;
        }

        private void piclmg_connet_Click(object sender, EventArgs e)
        {
            if (comPort.BaseStream.IsOpen)
            {
                doDisconnect(comPort);
            ...
            panel_NewConnet.Left = this.Width - panel_NewConnet.Width - 20;
            panel_NewConnet.Top = panel1.Bottom + 5;
            comboBox1_DropDown(null, null);
            if(panel_NewConnet.Visible)
                panel_NewConnet.Visible = false;
            else
                panel_NewConnet.Visible = true;


        }
        private void ConnetRunning(bool onLineflag)
        {
            if (!onLineflag)
            {
                this.piclmg_connet.Image = Properties.Resources.ConnetOK;
            }
            else
            {
                this.piclmg_connet.Image = Properties.Resources.ConnetNG;
            }
            //this.comboBox1.Enabled = !onLineflag;
            //this.comboBox2.Enabled = !onLineflag;
        }
        int messagecount;
        private void timer_warning_Tick(object sender, EventArgs e)
        {
          //  var newmsgcount = MainV2.comPort.MAV.cs.messages.Count;
            ...
            //        });
            //        //label_Warning.Text = MainV2.comPort.MAV.cs.messages[0].time.ToString() + " : " 
            //        //    + MainV2.comPort.MAV.cs.messages[0].message;
        ...
        private Image RectImage = null;

        private void MainV2_SizeChanged(object sender, EventArgs e)
        {
            addPosition();
        }
        private void Draw(RectangleF rectangle, Graphics g, int _radius)
        {
        ...
        {
            int count = MainV2.comPort.MAV.cs.messagerror.Count;                  
            if((MainV2.comPort.MAV.cs.gpsstatus2<2)&&(MainV2.comPort.MAV.cs.gpsstatus < 2))
                MainV2.comPort.MAV.cs.updateErrordata(101, "No Fix");
            else if ((MainV2.comPort.MAV.cs.satcount2 < 15)&&(MainV2.comPort.MAV.cs.satcount < 15))
                MainV2.comPort.MAV.cs.updateErrordata(100, "Bad GPS");
            if ((MainV2.comPort.MAV.cs.current >5)&&(!MainV2.comPort.MAV.cs.armed))
                MainV2.comPort.MAV.cs.updateErrordata(102, "Current Abnormality");

            if (count < MainV2.comPort.MAV.cs.messagerror.Count)
                return true;
            else
                return false;
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (!MainV2.comPort.MAV.cs.armed) // 已锁定
                {
                    ARME_text.Text = "Disarmed";
                    ARME_text.ForeColor = Color.Green;
                ...
                {
                    ARME_text.ForeColor = Color.Green;
                    ARME_text.Text = "Armed";///已解锁
                }
            }
            imageLabel_xinhao.Text = Convert.ToString((int)MainV2.comPort.MAV.cs.linkqualitygcs) + "%";
            Label_satellite.Text = Convert.ToString((int)MainV2.comPort.MAV.cs.satcount);
            HashSet<string> hsset = new HashSet<string>();
            StringBuilder message = new StringBuilder();
            var newmsgcount = MainV2.comPort.MAV.cs.messagerror.Count;
            bool redflag = checkerror();
            bool yellowflag = false;
            DateTime timeflag = DateTime.Now.AddSeconds(-31);
            if ((newmsgcount >0)||(redflag))
            {                
                try
                {
                    foreach (KeyValuePair<int, string> pair in MainV2.comPort.MAV.cs.messagerror)
                    {
                        hsset.Add(pair.Value+ "\r\n");
                        if (pair.Value.IndexOf("NO RC Receiver") >= 0)
                        {
                            yellowflag = true;
                        }
                        else
                        {
                            pictureBox2.Image = Resources._128_128_红色;
                            redflag = true;
                        }
            ...
            MainV2.comPort.MAV.cs.messages.ForEach(x =>
            {
                if (( x.Item1 > timeflag)&&(x.Item2.IndexOf("PreArm")>=0))
                    hsset.Add(x.Item2 + "\r\n");
                if (x.Item2.IndexOf("NO RC Receiver") >= 0)
                {
                    yellowflag = true;
                }
            });
            ...
            if (!MainV2.comPort.MAV.cs.armed) // 已锁定
            {
                //if(MainV2.comPort.MAV.cs.ch3in<980)
                //{
                //    yellowflag = true;
                //    message.Append("No RC Receiver \r\n");
                //}
                if ((MainV2.comPort.MAV.cs.satcount2 < 20) && (MainV2.comPort.MAV.cs.satcount2 > 15))
                {
                    yellowflag = true;
                    message.Append("GPS not healthy \r\n");
                }         
                if (MainV2.comPort.MAV.cs.battery_voltage <46.5)
                {
                    yellowflag = true;
                    message.Append("Bad Battery \r\n");
                }
               else if (MainV2.comPort.MAV.cs.battery_voltage < 48)
                {
                    yellowflag = true;
                    message.Append("Battery Low Voltage \r\n");
                }
               
                if (MainV2.comPort.MAV.cs.airspeed > 8)
                {
                    yellowflag = true;
                    message.Append("High airspeed, need calibration \r\n");
                }
                if((Math.Abs( MainV2.comPort.MAV.cs.roll)> 15) ||(Math.Abs(MainV2.comPort.MAV.cs.pitch) > 15))
                {
                    yellowflag = true;
                    message.Append("Bad attitude, level the UAV\r\n");
                }
                if (MainV2.comPort.MAV.param["MIS_TOTAL"] == null)
                {
                    yellowflag = true;
                    message.Append("no waypoint\r\n");
                }
                else
                {
            ...
            foreach (string sss in hsset)
                message.Append(sss);
            textBox_wranlog.Text = message.ToString();            
            MainV2.comPort.MAV.cs.messagerror.Clear();
            messagecount = newmsgcount;
            if (!redflag && yellowflag)
                pictureBox2.Image = Resources._128_128_黄色;

        }

        private void pictureBox2_Click(object sender, EventArgs e)
        {
            panel_waringlog.Visible = !panel_waringlog.Visible;
            if(panel_waringlog.Visible)
            {
                panel_waringlog.Left = pictureBox2.Right - panel_waringlog.Width;
                panel_waringlog.Top = panel1.Bottom + 5;
            }
        }

        private void pictureBox3_Click(object sender, EventArgs e)
        {
            panel_waringlog.Visible = false;
            pictureBox2.Image = null;
        }
        private Point offset = new Point();
        private void picture_top_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        private void picture_top_MouseMove(object sender, MouseEventArgs e)
        {
            if (MouseButtons.Left != e.Button) return;

            Point cur = MousePosition;
            panel_waringlog.Left = cur.X - offset.X;
            panel_waringlog.Top = cur.Y - offset.Y;
        }

    ...
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public FlightData()
        {
            ...
            */
            ...
            //    drawnpolygonsoverlay = new GMapOverlay("polygons");
            drawnpolygon = new GMapPolygon(new List<PointLatLng>(), "mypolygon");
            ...
            myhud.groundColor1 = Color.FromArgb(112, 132,49);
            myhud.groundColor2 = Color.FromArgb(146, 171, 48);
            myhud.skyColor1 = Color.FromArgb(92, 113, 174);
            myhud.skyColor2 = Color.FromArgb(168, 206,219);
            myhud.hudcolor = ThemeManager.HudText;
        ...
        private void BUT_ARM_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               if (MainV2.comPort.MAV.cs.alt > 10.0f)
                {
                    return;
                }
               //else
               // {
               //     MessageBox.Show(MainV2.comPort.MAV.cs.alt.ToString());
               // }
        ...
        private void BUT_loadtelem_Click(object sender, EventArgs e)
        {
            if (MainV2.comPort.BaseStream.IsOpen)
            {
                using (MessboxShowInfo messbox = new MessboxShowInfo("error", "The operation is denied when the UAV is connected", 3, 2, false))
                {
                    messbox.ShowDialog();
                }
                return ;
        ...
        private void BUT_quickmanual_Click(object sender, EventArgs e)
        {
            if (!MainV2.comPort.BaseStream.IsOpen)
                return;
            string modestr = MainV2.comPort.MAV.cs.mode;
            try
            {                
                ((Control) sender).Enabled = false;
                ...
                {
                    if ((modestr == "QSTABILIZE") || (modestr == "QHOVER") || (modestr == "QLOITER") || (modestr == "QLAND") ||  (modestr == "QRTL")|| (modestr == "Protect"))
                    {
        ...
        {///RTL
            ...
            try
            {
                ...
                //if (MainV2.comPort.MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)//gudingyi
                //{
                //    if ((modestr == "QSTABILIZE") || (modestr == "QHOVER") || (modestr == "QLOITER") || (modestr == "QLAND") || (modestr == "Protect"))
                ...
                //    MainV2.comPort.setMode("RTL");
                //}
                boxRTL = new MessboxShowInfo("Prompt", "RTL has been executed", 1);
                boxRTL.Show();
                checkindex = 1;
                timer_check_Count = 0;
        ...
        private void FlightData_Load(object sender, EventArgs e)
        {
            ...
            IsFlyTrac = CHK_autopan.Checked;
            CheckFlyTract(IsFlyTrac);
        ...
        private void flyToHereAltToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (MainV2.comPort.MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)//gudingyi
                {
                    if ((modestr == "QSTABILIZE") || (modestr == "QHOVER") || (modestr == "QLOITER") || (modestr == "QLAND") || (modestr == "QRTL") || (modestr == "Protect"))
                    {
                        CustomMessageBox.Show("Mode error ,No action", Strings.ERROR);
                        return;
                    }
                }
            }
            ...
            if (!MainV2.comPort.BaseStream.IsOpen)
            {
                MessboxShowInfo info = new MessboxShowInfo("Error", "Please connect your UAV first!", 3, 1);
                info.Show();
                return;
            }
            alt = MainV2.comPort.MAV.cs.alt.ToString("0");
          // MessageBox.Show(alt);
            using (MessboxInput messbox = new MessboxInput("Enter Guided Mode Alt", int.Parse(alt)))
            {
                messbox.ShowDialog();
                if (messbox.ShowMessageInfo)
                    alt = messbox.ALT.ToString();
                else
                    return;
            }
            //if (DialogResult.Cancel == MessboxInput.Show("Enter Alt", "Enter Guided Mode Alt", ref alt))
            //    return;
            ...
            //MessageBox.Show(alt);
           // MessageBox.Show(MainV2.comPort.MAV.GuidedMode.z.ToString());
        ...
        double WPdistall = 0.0f;
        double WPtraval = 0.0f;
        int maxflayalt = 10;
                ...
                try
                {
                    ...
                    if (tracklast.AddSeconds(Settings.Instance.GetDouble("FD_MapUpdateDelay", 0.3)) < DateTime.Now)
                    {
                        ...
                        if (waypoints.AddSeconds(5) < DateTime.Now)
                        {
                                ...
                                if (wps.Count >= 1)
                                {
                                    ...
                                    if (overlay.pointlist.Count > 0)
                                    {
                                        ...
                                        foreach (var plla in overlay.pointlist)
                                        {
                                            ...
                                            if (maxflayalt < plla.Alt)
                                                maxflayalt = (int)plla.Alt;
                                            distall += dist;
                                        ...
                                        //progressBar1.Maximum = (int)distall;
                                        //progressBar1.Minimum = 0;
                                        WPdistall = distall + homeplla.GetDistance(overlay.pointlist.First());
                                    }
                                    travdist -= MainV2.comPort.MAV.cs.wp_dist;
                                    WPtraval = travdist;
                                    if (MainV2.comPort.MAV.cs.mode.ToUpper() == "AUTO")
                                    {
                                        progressBar1.Value = (int)travdist;
                                        distanceBar1.traveleddist = (float)travdist;
                                    }
                                    //   progressBar1.Value = (int)MainV2.comPort.MAV.cs.distTraveled;                               
                                }
                            }
                            catch (Exception e)
                            {
                                log.Error("Draw wp error:______" +e.ToString());
                            }
                            
                            if(MainV2.comPort.MAV.cs.mode.ToUpper() == "DISARMED")
                            {
                                WPtraval = 0;

                                distanceBar2.totaldist = 0.0f;
                                distanceBar2.traveleddist = 0.0f;
                                progressBar1.Value = (int)0;
                                progressBar1.Text = string.Format("{0:f1}%", 0);
                            }
                            ...
                            //Console.WriteLine(GlobalVariable.Drawpolygon.Count);
                            if (m_Drawpolygon.Count > 0)
                            {

                                redrawPolygonSurvey(m_Drawpolygon);
                            }
                            else
                            {
                            ...
                            foreach (var port in MainV2.Comports.ToArray())
                            {
                                ...
                                foreach (var MAV in port.MAVlist)
                                {
                                    ...
                                    marker.ToolTipText = 
                                        ...
                                        + "Airspeed: " + MainV2.comPort.MAV.cs.airspeed.ToString("F2") + "m/s\r\n"
                                        + "Altitude : " + MainV2.comPort.MAV.cs.alt.ToString("F2") + "m\r\n"
                                        + "    Power: " + MainV2.comPort.MAV.cs.battery_voltage.ToString("F2") + "v\r\n"
                                        + "  current: " + MainV2.comPort.MAV.cs.current.ToString("F2") + "A";
                                    marker.ToolTipMode = MarkerTooltipMode.Always;
        ...
        public void redrawPolygonSurvey(List<PointLatLngAlt> list)
        {
            ...
            {
                drawnpolygon.Points.Add(new PointLatLng(list[i].Lat, list[i].Lng));
            }

            drawnpolygon.Fill = new SolidBrush(Color.FromArgb(50, Color.Green));
            // drawnpolygon.Fill = Brushes.Transparent;
            drawnpolygon.Stroke = new Pen(Color.Red, 3);
        ...
        private void addPosition()
        {
            ...
            picture_flytrac.Left = gMapControl1.Left + 3;
            picture_flytrac.Top = gMapControl1.Bottom - 5 - picture_flytrac.Height;

            pictureBox_minus.Left = gMapControl1.Right - 10 - pictureBox_minus.Width;
            pictureBox_minus.Top = picture_flytrac.Bottom - pictureBox_minus.Height;

            pictureBox_add.Left = pictureBox_minus.Left;
            pictureBox_add.Top = pictureBox_minus.Top - pictureBox_add.Height +2;

            userHSI1.Left = picture_flytrac.Left;
            userHSI1.Top = picture_flytrac.Top - userHSI1.Height - 5;
        }
        ...
        {
            pictureBox_add.Image = Properties.Resources.addMouseClick;
        }

        private void pictureBox_add_MouseLeave(object sender, EventArgs e)
        {
            pictureBox_add.Image = Properties.Resources.addMouseNomo ;
        }

        private void pictureBox_minus_MouseEnter(object sender, EventArgs e)
        {
            pictureBox_minus.Image = Properties.Resources.minusClick;
        }
        ...
        private void HidetableContrl()
        {
            tabControl_Newactions.ItemSize = new Size(0, 1);
            tabControl_Newactions.Appearance = TabAppearance.FlatButtons;
            tabControl_Newactions.SizeMode = TabSizeMode.Fixed;            
        }
        private System.Windows.Forms.Label selectLabel = null;
        private int selectindex = 0;
         private Color selectIndedx_color = Color.FromArgb(219, 130, 59);
      //  private Color selectIndedx_color = Color.Black;
        private Color NormalIndedx_color = Color.Transparent;
        private void label_Status_Click(object sender, EventArgs e)
        {
            selectLabel = label_Status;
            selectindex = 0;
            selectShow();
            tabControl_Newactions.SelectedIndex = selectindex;
        }

        private void label_Actions_Click(object sender, EventArgs e)
        {
            selectLabel = label_Actions;
            selectindex = 1;
            selectShow();
            tabControl_Newactions.SelectedIndex = selectindex;
        }

        private void label_Logs_Click(object sender, EventArgs e)
        {
            selectLabel = label_Logs;
            selectindex = 2;
            selectShow();
            tabControl_Newactions.SelectedIndex = selectindex;
        }

        private void label_Status_MouseEnter(object sender, EventArgs e)
        {
            System.Windows.Forms.Label ctrl = sender as System.Windows.Forms.Label;
            ctrl.BackColor = selectIndedx_color;
        }

        private void label_Status_MouseLeave(object sender, EventArgs e)
        {
            System.Windows.Forms.Label ctrl = sender as System.Windows.Forms.Label;
            ctrl.BackColor = NormalIndedx_color;
            selectShow();
        }
        public void selectShow()
        {
            label_Status.BackColor = NormalIndedx_color;
            label_Actions.BackColor = NormalIndedx_color;
            label_Logs.BackColor = NormalIndedx_color;
            label_Messages.BackColor = NormalIndedx_color;


            label_Status.ForeColor = Color.Black;
            label_Actions.ForeColor    = Color.Black;
            label_Logs.ForeColor    = Color.Black;
            label_Messages.ForeColor    = Color.Black;

            label_flytime.ForeColor = Color.Black;
            label_DistToHome1.ForeColor = Color.Black;
            label_Traveled.ForeColor = Color.Black;
            label_waypoint.ForeColor = Color.Black;
            label_wpnonew.ForeColor = Color.Black;
            switch (selectindex)
            {
                case 0:
                    label_Status.BackColor = selectIndedx_color;
                    break;
                case 1:
                    label_Actions.BackColor = selectIndedx_color;
                    break;
                case 2:
                    label_Logs.BackColor = selectIndedx_color;
                    break;
                case 3:
                    label_Messages.BackColor = selectIndedx_color;
                    break;
            }
        }
        ...
        public void IniUI()
        {
            Stream strme = GetType().Assembly.GetManifestResourceStream("MissionPlanner.Resources.backhsi.svg");
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(strme);
            var doc1 = SvgDocument.Open(xmlDoc);
            //    var doc1 = SvgDocument.Open(@"C:\Users\user01\Desktop\Rotate\WindowsFormsApp1\backhsi.svg");
            int wid = 512;
            int hei = 512;
            HSI1BackgroundImage = new Bitmap(wid, hei);
            doc1.Draw(HSI1BackgroundImage);
            userHSI1.BackgroundImage = HSI1BackgroundImage;
            strme = GetType().Assembly.GetManifestResourceStream("MissionPlanner.Resources.ui.hsihead.svg");
            xmlDoc = new XmlDocument();
            xmlDoc.Load(strme);
            userHSI1.SetImg = xmlDoc;
            myArcHalfProgressBar_battery.BackColor = Color.FromArgb(38, 39, 40);
            myArcHalfProgressBar_Throttle.BackColor = Color.FromArgb(38, 39, 40);
            myArcHalfProgressBar_Altitude.BackColor = Color.FromArgb(38, 39, 40);
            myArcHalfProgressBar_VerticalSpeed.BackColor = Color.FromArgb(38, 39, 40);
        }
        Bitmap HSI1BackgroundImage = null;
        private void label_Throttle_TextChanged(object sender, EventArgs e)
        {
            try
            {
                int persend = (int)(MainV2.comPort.MAV.cs.ch3percent11*100.0f);
                if (persend < 0)
                    persend = 0;
                else if (persend > 100)
                    persend = 100;
                myArcHalfProgressBar_Throttle.Value = persend;
                myArcHalfProgressBar_Throttle.PText = persend.ToString();
            }
            catch { }            
        }

        private void label_Battery_TextChanged(object sender, EventArgs e)
        {
            try
            {
                int persend = (int)(MainV2.comPort.MAV.cs.battery_remaining);
                if (persend < 0)
                    persend = 0;
                else if (persend > 100)
                    persend = 100;
                myArcHalfProgressBar_battery.Value = persend;
                myArcHalfProgressBar_battery.PText = persend.ToString();
            }
            catch { }
        }

        private void label_Altitude_TextChanged(object sender, EventArgs e)
        {
            try
            {
                int persend = (int)(MainV2.comPort.MAV.cs.alt);
                if (persend < 0)
                    persend = 0;
               
                else if (persend < 100)
                {
                    myArcHalfProgressBar_Altitude.UnitText = "M";
                    myArcHalfProgressBar_Altitude.MaxValue = maxflayalt;
                    myArcHalfProgressBar_Altitude.MinValue = 0;
                }
                else if (persend < 1000)
                {
                    myArcHalfProgressBar_Altitude.UnitText = "M";
                    myArcHalfProgressBar_Altitude.MaxValue = maxflayalt;
                    myArcHalfProgressBar_Altitude.MinValue = 0;
                ...
                {
                    persend /= 1000;
                    myArcHalfProgressBar_Altitude.UnitText = "KM";
                    myArcHalfProgressBar_Altitude.MaxValue = maxflayalt/1000;
                    myArcHalfProgressBar_Altitude.MinValue = 0;
                    
                }
                myArcHalfProgressBar_Altitude.Value = persend;
                myArcHalfProgressBar_Altitude.PText = persend.ToString();
            }
            catch { }
        }

        private void label_VerticalSpeed_TextChanged(object sender, EventArgs e)
        {
            try
            {
                double persend = MainV2.comPort.MAV.cs.verticalspeed;
                

                myArcHalfProgressBar_VerticalSpeed.Value = persend;
                myArcHalfProgressBar_VerticalSpeed.PText = persend.ToString("F1");
            }
            catch { }
        }

        private void gMapControl1_SizeChanged(object sender, EventArgs e)
        {
            addPosition();
        }

        private void label_homedist_TextChanged(object sender, EventArgs e)
        {
            if(WPdistall<1)
            {
             ...
             progressBar1.Maximum = (int)WPdistall;
             progressBar1.Minimum = 0;



            double valuedist = WPtraval;
            double persent = 100.0f * valuedist / progressBar1.Maximum;
            if (persent > 100.0f)
                persent = 100;
            progressBar1.Value = (int)valuedist;
            progressBar1.Text = string.Format("{0:f1}%", persent);
            ...
            {
                distanceBar2.totaldist = (float)WPdistall;
                distanceBar2.traveleddist = (float)WPtraval;
            }
        ...
        {
            //MessageBox.Show("hehkgqka1");
            // 空速校准
            string modecommdstr = "PREFLIGHT_CALIBRATION";
            if (!MainV2.comPort.BaseStream.IsOpen)
                return;
          //  MessageBox.Show("hehkgqka2");
            if (MainV2.comPort.MAV.cs.armed)//已解锁
            {
                return;
            }
          //  MessageBox.Show("hehkgqka"); 
            try
            {
                using (MessboxShowInfo box = new MessboxShowInfo("PREFLIGHT_CALIBRATION", "Whether to calibrate the airtachometer", 2))
                {
                    box.ShowDialog();
                    if (!box.ShowMessageInfo)
                        return;
                }
             //   MessageBox.Show("hehkgqka___ok");
                int param1 = 0;
                int param3 = 1;
                if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
                    param1 = 1; // gyro
                MainV2.comPort.doCommand((byte)MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, 
                    (MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), modecommdstr), param1, 0, param3, 0, 0, 0, 0);
            ...
            {
                using (MessboxShowInfo box = new MessboxShowInfo("Prompt", "PREFLIGHT_CALIBRATION error", 3))
                {
                    box.ShowDialog();
                }
            }
        ...
        private void tabControl_Newactions_SizeChanged(object sender, EventArgs e)
        {           
            var size = tabControl_Newactions.Size;
            int height,width,width1;
            width = (size.Width - 10);
            width1 = width / 4 - 16;
            height = (width1/2) + 50;
            tableLayoutPanel1.Left = 5;
            tableLayoutPanel1.Width = width;
            tableLayoutPanel1.Top = (size.Height - height) / 2;
            tableLayoutPanel1.Height = height;

            panel_actions.Left = (size.Width - panel_actions.Width) / 2;
            panel_actions.Top  = (size.Height - panel_actions.Height) / 2;

            panel_logs.Left = (size.Width - panel_logs.Width) / 2;
            panel_logs.Top = (size.Height - panel_logs.Height) / 2 + 20;

        }
        public void modifyandSetAltMaxAndMin(float max,float min)
        {
            try
            {
                modifyandSetAlt.Maximum = (Decimal)max;
                modifyandSetAlt.Minimum = (Decimal)min;
            }
            catch
            {
        ...
        public void modifyandSetLoiterRadMaxAndMin(float max, float min)
        {
            try
            {
                modifyandSetLoiterRad.Maximum = (Decimal)max;
                modifyandSetLoiterRad.Minimum = (Decimal)min;
            }
            catch
            {
        ...
        private bool IsFlyTrac = false;
        private void picture_flytrac_Click(object sender, EventArgs e)
        {
            IsFlyTrac = !IsFlyTrac;
            CheckFlyTract(IsFlyTrac);
        }
        private void CheckFlyTract(bool check)
        {
            if (check)
                picture_flytrac.Image = Resources.触发状态;
            else
                picture_flytrac.Image = Resources.原始状态;
            CHK_autopan.Checked = check;
        }

        private void label_Messages_Click(object sender, EventArgs e)
        {
            selectLabel = label_Messages;
            selectindex = 3;
            selectShow();
            tabControl_Newactions.SelectedIndex = selectindex;
        }

        //private void label_Actions_MouseEnter(object sender, EventArgs e)
        //{
        //    label_Actions.BackColor = selectIndedx_color;
        //}

        //private void label_Actions_MouseLeave(object sender, EventArgs e)
        //{
        //    if (1 == selectindex)
        //        label_Actions.BackColor = selectIndedx_color;
        //    else
        ...
        //private void label_Logs_MouseEnter(object sender, EventArgs e)
        //{
        //    label_Logs.BackColor = selectIndedx_color;
        //}

        //private void label_Logs_MouseLeave(object sender, EventArgs e)
        //{
        //    if (2 == selectindex)
        //        label_Logs.BackColor = selectIndedx_color;
        //    else
        //        label_Logs.BackColor = NormalIndedx_color;
        //}
        List<PointLatLngAlt> m_Drawpolygon = new List<PointLatLngAlt>();
        public void SetDrawPolygonDate(List<PointLatLng> info)
        {
            List<PointLatLngAlt> Tlist = new List<PointLatLngAlt>();
            for(int i = 0;i< info.Count;i++)
            {
                Tlist.Add(new PointLatLngAlt(info[i]));
            }
            m_Drawpolygon = Tlist;
        }
        ...
        private void follwMeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new FollowMe().Show();
        }
        ...
        int timer_check_Count = 0;
        private void timer_check_Tick(object sender, EventArgs e)
        {           
            timer_check_Count++;
            if (1 == checkindex) ///rtl 处理
            {
                if (timer_check_Count > 3)
                {
                    timer_check.Stop();
                    timer_check_Count = 0;
                    boxRTL.Close();
            ...
            else ///空速处理
            {
                if (timer_check_Count > 3)
                {
                    timer_check.Stop();
                    timer_check_Count = 0;
                    if (MainV2.comPort.MAV.cs.airspeed > 3.0f)
                    {
                        using (MessboxShowInfo box = new MessboxShowInfo("PREFLIGHT_CALIBRATION", "Airspeedometer calibrated error", 3))
                        {
                            box.ShowDialog();
                        }
                    }
                    ...
                    {
                        using (MessboxShowInfo box = new MessboxShowInfo("PREFLIGHT_CALIBRATION", "Airspeedometer calibrated successfully", 5))
                        {
                            box.ShowDialog();
                        }
                    }

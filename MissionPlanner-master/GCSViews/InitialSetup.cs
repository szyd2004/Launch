...
namespace MissionPlanner.GCSViews
{
    ...
    BackstageViewPage Parent = null, bool advanced = false)
        ...
        private BackstageViewPage AddBackstageViewPage1(Type userControl, string headerText,
            BackstageViewPage Parent = null, bool advanced = false)
        {
            try
            {
        ...
        public void LoadConfig()
        {
            try
            {
                if(backstageView.Items.Count>0)
                {
                    foreach(BackstageViewPage itm in backstageView.Items)
                        if(itm.Text.IndexOf(Strings.Planner)>=0)
                        {
                            return;
                        }
                }
                ...
                if (gotAllParams)
                {
                    if (MainV2.comPort.BaseStream.IsOpen)
                    {
                        //if (MainV2.DisplayConfiguration.displayFlightModes)
                        //{
                        //    start = AddBackstageViewPage1(typeof(ConfigFlightModes), Strings.FlightModes);
                        //}

                        //if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
                        //    AddBackstageViewPage1(typeof(ConfigAC_Fence), Strings.GeoFence);

                        if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
                        {
                            if (MainV2.DisplayConfiguration.displayBasicTuning)
                            {
                                start = AddBackstageViewPage1(typeof(ConfigSimplePids), Strings.BasicTuning);
                            }

                            //if (MainV2.DisplayConfiguration.displayExtendedTuning)
                            //{
                            //    AddBackstageViewPage1(typeof(ConfigArducopter), Strings.ExtendedTuning);
                            //}
                        }

                        if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduPlane)
                        {
                            start = AddBackstageViewPage1(typeof(ConfigArduplane), Strings.BasicTuning);
                        }

                        if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduRover)
                        {
                            start = AddBackstageViewPage1(typeof(ConfigArdurover), Strings.BasicTuning);
                        }

                        //if (MainV2.comPort.MAV.cs.firmware == Firmwares.ArduTracker)
                        //{
                        //    start = AddBackstageViewPage1(typeof(ConfigAntennaTracker), Strings.ExtendedTuning);
                        //}

                        if (MainV2.DisplayConfiguration.displayBasicTuning)
                        {
                            AddBackstageViewPage1(typeof(ConfigFriendlyParams), Strings.StandardParams);
                        }

                        //if (MainV2.DisplayConfiguration.displayAdvancedParams)
                        //{
                        //    AddBackstageViewPage1(typeof(ConfigFriendlyParamsAdv), Strings.AdvancedParams, null, true);
                        //}

                        //if (!Program.MONO && ConfigOSD.IsApplicable())
                        //{
                        //    AddBackstageViewPage1(typeof(ConfigOSD), Strings.OnboardOSD);
                        //}

                        //if ((MainV2.comPort.MAV.cs.capabilities & (int)MAVLink.MAV_PROTOCOL_CAPABILITY.FTP) > 0)
                        //    AddBackstageViewPage1(typeof(MavFTPUI), Strings.MAVFtp);

                        //if (true)
                        //{
                        //    AddBackstageViewPage1(typeof(ConfigUserDefined), Strings.User_Params);
                        //}
                    }
                }
                ...
                if (MainV2.DisplayConfiguration.displayFullParamList)
                {
                    if (!MainV2.comPort.BaseStream.IsOpen || gotAllParams)
                        AddBackstageViewPage1(typeof(ConfigRawParams), Strings.FullParameterList, null, true);
                }
                if (MainV2.DisplayConfiguration.displayFullParamTree && !Program.MONO)
                {
                    if (!MainV2.comPort.BaseStream.IsOpen || gotAllParams)
                        AddBackstageViewPage1(typeof(ConfigRawParamsTree), Strings.FullParameterTree, null, true);
                }
                ...
                if (MainV2.comPort.BaseStream.IsOpen)
                {
                    ...
                    {
                        start = AddBackstageViewPage1(typeof(ConfigFlightModes), Strings.FlightModes);
                        AddBackstageViewPage1(typeof(ConfigAteryxSensors), "Ateryx Zero Sensors");
                        AddBackstageViewPage1(typeof(ConfigAteryx), "Ateryx Pids");
                    }

                    if (!gotAllParams)
                    {
                        if (start == null)
                            start = AddBackstageViewPage1(typeof(ConfigParamLoading), Strings.Loading);
                        else
                            AddBackstageViewPage1(typeof(ConfigParamLoading), Strings.Loading);
                    }

                    AddBackstageViewPage1(typeof(ConfigPlanner), Strings.Planner);
                }
                else
                {
                    start = AddBackstageViewPage1(typeof(ConfigPlanner), Strings.Planner);
                }

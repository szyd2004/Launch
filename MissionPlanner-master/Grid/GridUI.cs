...
namespace MissionPlanner.Grid
{
    public partial class GridUI : Form
    {
        ...
        void loadgriddata(GridData griddata)
        {
            ...
            NUM_leadin.Value = NUM_overshoot.Value;
        ...
        void loadsettings()
        {
            if (plugin.Host.config.ContainsKey("grid_camera"))
            {
                ...
                /// add new 
                loadsetting("grid_MiniLine_spacing", numericUpDown1);
                loadsetting("grid_BufferPoint", numericUpDown_BufferPoint);
            }
        }
        ...
        void savesettings()
        {
            ...
            ///add new 
            plugin.Host.config["grid_MiniLine_spacing"] = numericUpDown1.Value.ToString();
            plugin.Host.config["grid_BufferPoint"]           = numericUpDown_BufferPoint.Value.ToString();
        }
        ...
        private async void domainUpDown1_ValueChanged(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (DistUnits == "Feet")
            {
                ...
                textBox_grndres.Value = decimal.Parse(inchpixel);
            ...
            else
            {
                ...
                string tempstr = TXT_cmpixel.Text;
                tempstr = tempstr.Replace("cm", "");
                textBox_grndres.Value = decimal.Parse(tempstr);
        ...
        private void BUT_Accept_Click(object sender, EventArgs e)
        {
            ...
            if (grid != null && grid.Count > 0)
            {
                ...
                MainV2.instance.FlightPlanner.ShowDrawAdd();

                MainV2.instance.FlightPlanner.InsertCalibrationPoint((int)numericUpDown_BufferPoint.Value, false);
        ...
        private void myButton_GR_Click(object sender, EventArgs e)
        {
            float focallen = (float)NUM_focallength.Value;
            // float flyalt = (float)CurrentState.fromDistDisplayUnit((float)NUM_altitude.Value);
            float flyGrares = (float)(textBox_grndres.Value);
            int imageheight = int.Parse(TXT_imgheight.Text);

            float sensorheight = float.Parse(TXT_sensheight.Text);

            float tempdate = (flyGrares * imageheight * focallen) / (100 * sensorheight);

            NUM_altitude.Value = Convert.ToInt32(tempdate);
        }

        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {// Min Lane separation = 最小行间距 / distbetween （四舍五入取整写入）
            double tempvalue = (double)numericUpDown1.Value /( (double)NUM_Distance.Value);
            NUM_Lane_Dist.Value = (int)Math.Round(tempvalue); 
        }
    }
}